...
std::vector<std::unique_ptr<column>> multi_contains_warp_parallel(strings_column_view const& input,
                                                                  ...
                                                                  rmm::mr::device_memory_resource* mr)
{
    ...
    std::transform(targets.begin(), targets.end(), host_device_targets.begin(),
                   [&](auto const& target){ return string_view{target.get().data(), target.get().size()}; });
    host_device_targets.host_to_device_sync(stream);

    // Create output columns.
    auto const results_iter = thrust::make_transform_iterator(targets.begin(), [&](auto const& target) {
    ...
    auto results_list = std::vector<std::unique_ptr<column>>(results_iter, results_iter + targets.size());
    auto host_device_results_list = cudf::detail::hostdevice_vector<bool*>(num_targets, stream);
    std::transform(results_list.begin(), results_list.end(), host_device_results_list.begin(),
                   [&](auto const& results_column) { return results_column->mutable_view().template data<bool>(); }
    );
    ...
    multi_contains_warp_parallel_fn<<<grid.num_blocks, grid.num_threads_per_block, 0, stream.value()>>>(
